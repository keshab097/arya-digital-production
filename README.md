...
2. **All media is self-hosted in `public/img/`** (44 files, ~1.3 MB) so it loads from the site's own origin with no third-party runtime dependency. (The earlier version pulled from loremflickr/pravatar/Simple Icons CDNs, which were unreliable from some networks and left the live site without images.) Every `<img>` keeps `data-placeholder="true"` for easy find-and-swap; paths use `import.meta.env.BASE_URL` so they resolve at both the Pages subpath and a root domain.
   - **Photos** were sourced topically and bundled. 5 that the source returned flaky during download fell back to generic stock (featured, healthcare, mobility, and two blog covers) — they load fine but are less on-theme. Swap any by replacing the file in `public/img/` with the same name, then redeploy.
   - **Faces** (testimonials, team) are bundled portrait avatars (`face-*.jpg`).
   - **Brand logos** (tool stack, awards) are bundled Simple Icons SVGs in brand colors (`logo-*.svg`); brands Simple Icons dropped for trademark reasons (Ahrefs, LinkedIn, Adobe, Klaviyo, Surfer, Screaming Frog, Microsoft) render as wordmarks.
   - The duotone grayscale treatment was removed so the real colors show; `.work-image` applies only a gentle saturation grade.
